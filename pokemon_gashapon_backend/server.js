...
app.get("/api/pic_poke", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `http://13.228.191.168:3000/api/pic_poke?pok_name=${pokemonName}`
    );
    ...
    res.status(500).json({
      success: false,
      message: `Error fetching Pokémon image for ${pokemonName}`,
    });
  }
});
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    const insertSql =
      "INSERT INTO user_id (user_name, pass, consecutive_pulls) VALUES ($1, $2, 0) RETURNING user_id";
...
app.get("/api/user", async (req, res) => {
  try {
    ...
    const sql =
      "SELECT user_id, consecutive_pulls FROM user_id WHERE user_name = $1";
...
app.get("/api/random-pokemon", async (req, res) => {
  try {
    ...
    await client.query(
      "UPDATE user_id SET consecutive_pulls = consecutive_pulls + 1 WHERE user_id = $1",
      [userId]
    );

    // ดึงจำนวนครั้งที่สุ่มปัจจุบัน
    const pullsResult = await client.query(
      "SELECT consecutive_pulls FROM user_id WHERE user_id = $1",
      [userId]
    );
    ...
    if (consecutivePulls % 6 === 0) {
      result = await client.query(
        "SELECT * FROM pokemon WHERE poke_type_card = 'rare' ORDER BY RANDOM() LIMIT 1"
      );
      await client.query(
        "UPDATE user_id SET consecutive_pulls = 0 WHERE user_id = $1",
        [userId]
      );
      ...
      result = await client.query(`
        SELECT * FROM pokemon 
        WHERE RANDOM() < CASE 
          WHEN poke_type_card = 'rare' THEN 0.1 + ${
            Math.min(consecutivePulls * 0.5, 20) / 100
          }
    ...
    const imageResult = await client.query(
      "SELECT pok_image FROM pic_poke WHERE pok_name = $1",
      [randomPokemon.pok_name.trim()]
    );
...
app.post("/api/duplicate-pokemon", async (req, res) => {
  try {
    ...
    const checkResult = await client.query(
      "SELECT * FROM user_pokemon WHERE user_id = $1 AND pokemon_id = $2",
      [userId, pokemonId]
    );

    if (checkResult.rows.length > 0) {
      await client.query(
        "UPDATE user_pokemon SET power_up_points = power_up_points + 1 WHERE user_id = $1 AND pokemon_id = $2",
        [userId, pokemonId]
      );
      res.json({ message: "Duplicate Pokemon. Power-up points added." });
    } else {
      await client.query(
        "INSERT INTO user_pokemon (user_id, pokemon_id, power_up_points) VALUES ($1, $2, 0)",
        [userId, pokemonId]
      );
...
app.get("/api/user-collection", async (req, res) => {
  try {
    ...
    const collectionWithImages = result.rows.map((pokemon) => ({
      ...pokemon,
      image: pokemon.pok_image
        ? `data:image/png;base64,${pokemon.pok_image}`
        : null,
    }));
...
app.get("/api/user-pokemon-points", async (req, res) => {
  try {
    ...
    const sql =
      "SELECT power_up_points FROM user_pokemon WHERE user_id = $1 AND pokemon_id = $2";
